...
class PatchEmbedInit(nn.Module):
    def __init__(
        ...
        self.proj_lif = create_neuron(neuron_type=neuron_type, surrogate_type=surrogate_function, **neuron_args)
        ...
        self.proj1_lif = create_neuron(neuron_type=neuron_type, surrogate_type=surrogate_function, **neuron_args)
        ...
        self.proj2_lif = create_neuron(neuron_type=neuron_type, surrogate_type=surrogate_function, **neuron_args)
        ...
        self.proj_res_lif = create_neuron(neuron_type=neuron_type, surrogate_type=surrogate_function, **neuron_args)
...
class ChannelQKAttention(nn.Module):
    """
    QK Channel Attention (QKCA) module from QKFormer paper.

    This module computes channel-wise attention by summing across the spatial/token dimension
    of Q to create a channel attention vector, then applies it to K via element-wise multiplication.

    Mathematical formulation:
    Ac = SN(ΣN j=0 Qi, j) # Column summation
    X' = Ac ⊗ K # Channel mask operation
    """

    def __init__(
            self, dim, num_heads=8, neuron_type="LIF", surrogate_function="sigmoid", neuron_args=None, dropout=0.2
    ):
        super().__init__()
        ...
        self.q_lif = create_neuron(neuron_type=neuron_type, surrogate_type=surrogate_function, **neuron_args)

        # K pathway
        self.k_conv = nn.Conv1d(dim, dim, kernel_size=1, stride=1, bias=False)
        self.k_bn = nn.BatchNorm1d(dim)
        self.k_lif = create_neuron(neuron_type=neuron_type, surrogate_type=surrogate_function, **neuron_args)

        # Attention LIF neuron
        self.attn_lif = create_neuron(neuron_type=neuron_type, surrogate_type=surrogate_function, **neuron_args)
    ...
    def forward(self, x):
        ...
        q_conv_out = self.q_lif(q_conv_out)  # [T, B, C, N]

        # Generate K
        k_conv_out = self.k_conv(x_for_qkv)
        k_conv_out = self.k_bn(k_conv_out).reshape(T, B, C, N)
        k_conv_out = self.k_lif(k_conv_out)  # [T, B, C, N]

        # Reshape Q for multi-head attention
        q = q_conv_out.reshape(T, B, self.num_heads, self.head_dim, N)  # [T, B, num_heads, head_dim, N]

        # Channel attention: sum across spatial dimension (N)
        # According to the paper: Ac = SN(ΣN j=0 Qi,j)
        channel_attention = torch.sum(q, dim=4, keepdim=True)  # [T, B, num_heads, head_dim, 1]

        # Apply LIF neuron to get binary channel attention vector
        channel_attention = self.attn_lif(channel_attention)  # [T, B, num_heads, head_dim, 1]

        # Apply dropout after attention
        channel_attention = self.attn_dropout(channel_attention.to(dtype))

        # Reshape K for multi-head attention
        k = k_conv_out.reshape(T, B, self.num_heads, self.head_dim, N)  # [T, B, num_heads, head_dim, N]

        # Apply channel attention to K (channel mask operation)
        # This is the Ac ⊗ K operation from the paper
        x = torch.mul(channel_attention, k)  # [T, B, num_heads, head_dim, N]

        # Merge heads back
        x = x.reshape(T, B, C, N)  # [T, B, C, N]
...
class MLP(nn.Module):
    def __init__(
        ...
        self.fc1_lif = create_neuron(neuron_type=neuron_type, surrogate_type=surrogate_function, **neuron_args)

        self.fc2_conv = nn.Conv2d(hidden_features, out_features, kernel_size=1, stride=1)
        self.fc2_bn = nn.BatchNorm2d(out_features)
        self.fc2_lif = create_neuron(neuron_type=neuron_type, surrogate_type=surrogate_function, **neuron_args)
...
class PatchEmbeddingStage(nn.Module):
    def __init__(
        ...
        self.proj3_lif = create_neuron(neuron_type=neuron_type, surrogate_type=surrogate_function, **neuron_args)
        ...
        self.proj4_lif = create_neuron(neuron_type=neuron_type, surrogate_type=surrogate_function, **neuron_args)
...
class SpikingSelfAttention(nn.Module):
    def __init__(
        ...
        self.v_lif = create_neuron(neuron_type=neuron_type, surrogate_type=surrogate_function, **neuron_args)
        self.attn_lif = create_neuron(neuron_type=neuron_type, surrogate_type=surrogate_function, **neuron_args)
