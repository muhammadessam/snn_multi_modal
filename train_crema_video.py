...
def create_transforms(args, is_training=True):
    ...
    if is_training:
        ...
        # Increased magnitude for RandAugment and added Gaussian Blur for stronger regularization.
...
def create_dataloaders(args, _logger):
    """Create train and validation dataloaders using Imagenette."""
    val_transform = create_transforms(args, is_training=False)
    val_dataset = CremaVideo(
        data_path=args.data_path,
        num_frames=args.time_step,
    ...
    _logger.info(
        f"Using Imagenette dataset. Found {len(train_dataset)} training samples and {len(val_dataset)} validation samples."
    )
...
def get_args():
    ...
    parser.add_argument(
        "--data-path",
        type=str,
        default="/user/mohamed.saleh01/u18697/projects/datasets/crema/Crema_frames_8_90_cropped_128",
        help="Path to dataset root folder",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default="./out/crema/video",
        help="Path to save checkpoints and logs",
    )
    parser.add_argument("--model", default="qk_video", type=str, help="Name of model to train")
    parser.add_argument("--resume", default="", type=str, help="Path to latest checkpoint")
    parser.add_argument("--epochs", type=int, default=100, help="Number of base epochs to train")
    ...
    parser.add_argument(
        ...
        type=int,
        default=6,
        help="Number of label classes (for CREMA-D)",
    )
    ...
    parser.add_argument("--embed-dims", type=int, default=192, help="Embedd8ing dimension")

    parser.add_argument("--mlp-ratios", type=int, default=2, help="MLP hidden dimension ratio")
    parser.add_argument(
        "--depths",
        type=int,
        nargs="+",
    ...
    parser.add_argument("--patient", default=40, help="Early stopping patience")
    # --- Optimizer ---
    ...
    parser.add_argument("--warmup-epochs", type=int, default=20, help="Epochs to warmup LR")
    parser.add_argument("--warmup-lr", type=float, default=1e-5, help="Warmup learning rate")
    parser.add_argument("--min-lr", type=float, default=1e-5, help="Lower bound for LR")
    parser.add_argument("--cooldown-epochs", type=int, default=20, help="Cooldown epochs")
    ...
    parser.add_argument("--reprob", type=float, default=0.0, help="Random erase probability (disabled)")
    ...
    parser.add_argument("--mixup", type=float, default=0.2, help="Mixup alpha (increased)")
    parser.add_argument("--cutmix", type=float, default=0.2, help="Cutmix alpha (increased)")
    parser.add_argument(
        "--mixup-switch-prob",
        type=float,
        default=0.5,
    ...
    parser.add_argument(
        ...
        default=1000,
        type=int,
        help="Turn off mixup after this epoch",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="LIF",
        help="Type of neuron to use (e.g., LIF)",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="sigmoid",
        help="Type of surrogate function",
    )
...
def main():
    ...
    output_dir = get_outdir(
        args.output_dir,
        str(args.neuron_type)
        + "_"
        + str(args.surrogate_function)
        + "_"
        + str(args.time_step)
        + "_"
        + f"{datetime.datetime.now():%Y_%m_%d_%H_%M_%S}",
    )
    ...
    model = HierarchicalSpikingTransformer(
    ...
    loader_train, loader_eval = create_dataloaders(args, _logger)
...
def plot_metrics(train_accuracies, val_accuracies, train_losses, val_losses, output_dir, _logger):
    """Plots both accuracy and loss curves in a single figure with two subplots."""
    ...
    # --- Loss Plot ---
    ax2.plot(epochs_range, train_losses, label="Training Loss", marker="s", color="tab:blue")
    ax2.plot(
        epochs_range,
        val_losses,
    ...
    _logger.info(f"Metrics plot saved to: {plot_path}")
